...
<form id="expense-form" novalidate>
    ...
    <div class="row mb-3">
        <div class="col-md-3">
            ...
            <input type="date" class="form-control" id="date" required autofocus value="<?php echo date('Y-m-d'); ?>">
    ...
    <div class="mb-3">
        ...
        <div class="invalid-feedback">
            A descrição é obrigatória e deve ter no máximo 255 caracteres.
        </div>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-3">
            <label for="additional_discount" class="form-label">Desconto adicional</label>
            <input type="text" class="form-control currency" id="additional_discount" placeholder="R$ 0,00">
            <div class="form-text">Opcional. Usado apenas para conferência.</div>
        </div>
        ...
        <div class="col-md-3">
            ...
            <div class="form-text">
                Segure Ctrl (Windows) ou Command (Mac) para selecionar múltiplas.
            </div>
        </div>
    </div>
...
<script>
    $(function() {
        ...
        function loadExpense(id) {
            $.getJSON(API_BASE + '/expenses/' + id, function(e) {
                ...
                $('#additional_discount').val(
                    FH.formatCurrencyBRFromNumber(e.additional_discount)
                );
        ...
        $('#expense-form').on('submit', function(e) {
            ...
            if (!dateVal) markInvalid($date);
            if (!descCheck.valid) markInvalid($description);
            if (amountPaidVal === null) markInvalid($amountPaid);
            ...
            const payload = {
                supplier_id: $('#supplier_id').val()
                    ? parseInt($('#supplier_id').val(), 10)
                    : null,
                date: dateVal,
                description: descCheck.value,
                amount_nf: $('#amount_nf').val()
                    ? FH.parseCurrencyToFloat($('#amount_nf').val())
                    : null,
                amount_paid: amountPaidVal,
                additional_discount: $('#additional_discount').val()
                    ? FH.parseCurrencyToFloat($('#additional_discount').val())
                    : null,
                tags: ($('#tags_select').val() || []).map(t => parseInt(t, 10))
            };
